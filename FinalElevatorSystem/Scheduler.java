...
public class Scheduler {
	...
	InetAddress ElevatorAndFloorIP = InetAddress.getLocalHost();
	//Unquote line below and replace the "host" with ip address of Elevator and Floor
	//InetAddress ElevatorAndFloorIP = InetAddress.getByName(host);
	

	/*
	 * Constructor
	 */
	public Scheduler() throws UnknownHostException
	{
		...
		try {
			System.out.println("ElevatorAndFloorIP: "+ElevatorAndFloorIP);
			elevatorSocket = new DatagramSocket(sysctrl.getPort("SchedulerReceiveElevatorPort"),ElevatorAndFloorIP);	// socket for receiving floor arrival updates
			elevatorSocket2 = new DatagramSocket(sysctrl.getPort("ElevatorStatusPort"),ElevatorAndFloorIP);   // socket for receiving elevator status
			floorSocket = new DatagramSocket(sysctrl.getPort("floorSendPort"),ElevatorAndFloorIP); // socket for receiving floor request
